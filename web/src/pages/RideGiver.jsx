...
const RideGiver = () => {
  ...
  // 🔥 ADDED: STRICT LOCK LOGIC
  // Requirements: License (isDriver) + RC (rcVerified) + Aadhar (isAadharVerified) + Medical (isMedicalCompleted)
  ...
  if (!isProfileComplete) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="bg-white p-10 rounded-3xl shadow-xl text-center max-w-lg border border-red-100">
          <div className="text-6xl mb-4">🚫</div>
          <h2 className="text-2xl font-black text-slate-900 mt-2">
            Driver Mode Locked
          </h2>
          <p className="text-slate-500 mt-2 mb-6">
            To ensure safety, you must complete the following in your Profile:
          </p>
          <ul className="text-left text-sm space-y-2 bg-slate-50 p-4 rounded-xl mb-6">
            <li
              className={
                user?.isDriver ? "text-emerald-600 font-bold" : "text-slate-400"
              }
            >
              {user?.isDriver ? "✅" : "❌"} Driving License Verified
            </li>
            <li
              className={
                user?.rcVerified
            ...
            >
              {user?.rcVerified ? "✅" : "❌"} Vehicle RC Verified
            </li>
            <li
              className={
                user?.isAadharVerified
            ...
            >
              {user?.isAadharVerified ? "✅" : "❌"} Aadhar Verification
            </li>
            <li
              className={
                user?.isMedicalCompleted
            ...
            >
              {user?.isMedicalCompleted ? "✅" : "❌"} Medical History Filled
            </li>
          </ul>
          <button
            onClick={() => navigate("/profile")}
            className="w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition shadow-lg"
          >
        ...
        <div className="flex flex-col gap-4 min-w-[300px] p-2">
          <div className="flex items-center gap-3">
            ...
            <p className="text-lg font-bold text-slate-800">
              Post ride for{" "}
              <span className="text-emerald-600">₹{calculatedPrice}</span>?
            </p>
          </div>
  ...
  const executePostRide = async () => {
    ...
    try {
      ...
      const safeImageUrl =
        user.profileImage?.length > 100000
      ...
      await addDoc(collection(db, "rides"), {
        ...
        passengers: [],
        // 🔥 Saving verification status for Ride Taker trust
        driverVerified: true,
        rcVerified: true,
        aadharVerified: true,
      });
      ...
      console.error(error);
      toast.error("Failed to post ride. Image might be too large.", {
        id: toastId,
      });
    } finally {
      setLoading(false);
  ...
  if (!user)
    return <div className="p-10 text-center font-bold">Please Login First</div>;
    ...
    <div className="h-[85vh] max-h-[85vh] m-2 bg-slate-50 flex flex-col md:flex-row overflow-hidden border-b border-slate-200 shadow-sm">
      ...
      <div className="w-full md:w-1/3 p-4 md:p-6 bg-white border-r border-slate-200 overflow-y-auto z-10 custom-scrollbar">
        ...
        <p className="text-md text-slate-500 mb-4 font-medium ">
          Share your car, Save cost.
        </p>

        <form onSubmit={handleCalculateRoute} className="space-y-3">
          <div>
            <label className="block text-sm font-bold text-slate-500 mb-0.5 ml-1">
              Pickup Location
            </label>
            <input
              type="text"
          ...
          <div className="space-y-1.5 pt-1">
            <h3 className="text-sm font-bold text-slate-500  ml-1">
              Security Filters
            </h3>

            <div
              className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer transition-all ${sameGender ? "bg-emerald-50 border-emerald-500" : "bg-white border-slate-200"}`}
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p
                    className={`text-sm font-bold ${sameGender ? "text-emerald-900" : "text-slate-700"}`}
                  >
                    Same Gender Only
                  </p>
                  <p className="text-xs text-slate-400 font-medium tracking-tight">
                    Only {user.gender || "verified"} users
                  </p>
                </div>
              </div>
              <div
                className={`w-8 h-4 rounded-full relative ${sameGender ? "bg-emerald-500" : "bg-slate-300"}`}
              >
                <div
                  className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${sameGender ? "left-4.5" : "left-0.5"}`}
                />
              </div>
            </div>

            <div
              className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer transition-all ${sameInstitution ? "bg-blue-50 border-blue-500" : "bg-white border-slate-200"}`}
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p
                    className={`text-sm font-bold ${sameInstitution ? "text-blue-900" : "text-slate-700"}`}
                  >
              ...
              <div
                className={`w-8 h-4 rounded-full relative ${sameInstitution ? "bg-blue-500" : "bg-slate-300"}`}
              >
                <div
                  className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${sameInstitution ? "left-4.5" : "left-0.5"}`}
                />
              </div>
            </div>
          </div>
            ...
            <div className="bg-emerald-50 border border-emerald-100 p-3 rounded-xl relative overflow-hidden">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <h3 className="text-xl font-black text-emerald-900 leading-none mt-0.5">
                    ₹{calculatedPrice}
                    <span className="text-sm font-normal">/seat</span>
                  </h3>
                </div>
