...
const RideHistory = () => {
  ...
  // Helper to calculate cost dynamically if missing in document
  const getDynamicCost = (distance) => {
    const dist = parseFloat(distance || 0);
    return Math.round(20 + dist * 10); // ₹20 Base + ₹10/km
  };
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {/* --- PASSENGER VIEW (COST CALCULATION) --- */}
          {activeTab === "taker" &&
            (requests.length === 0 ? (
              <p className="text-slate-400 italic">No ride requests found.</p>
            ) : (
              requests.map((req) => {
                // Priority: Stored pricePerSeat > Calculate from distance > Fallback
                const fare =
                  req.pricePerSeat || getDynamicCost(req.distance || 0);
                ...
                return (
                  ...
                  >
                    ...
                    {req.status === "ACCEPTED" && (
                      <button
                        onClick={() =>
                          setPayModalData({ ...req, calculatedFare: fare })
                        }
                        className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition flex items-center gap-2"
                      >
                        <span>💸</span> Pay ₹{fare}
                      </button>
                    )}
          ...
          {/* --- DRIVER VIEW (EARNINGS CALCULATION) --- */}
          {activeTab === "giver" &&
            (postedRides.length === 0 ? (
              ...
              postedRides.map((ride) => {
                // Calculate per seat price and total earnings
                const seatPrice =
                  ride.pricePerSeat || getDynamicCost(ride.distance);
                const passengersCount = ride.passengers?.length || 0;
                const totalEarned = passengersCount * seatPrice;

                return (
                  <div
                    key={ride.id}
                    className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center group"
                  >
                    <div>
                      <h3 className="font-bold text-slate-900 text-lg">
                      ...
                      <div className="flex gap-4 mt-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        <span>👤 {passengersCount} Confirmed</span>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-500 text-xs mb-6 italic">
                Secure transaction via RouteOpt
              </p>

              <div className="bg-white p-4 rounded-2xl border-2 border-slate-900 inline-block shadow-inner mb-6">
                <QRCode
                  value={`upi://pay?pa=driver@upi&pn=RouteOptDriver&am=${payModalData.calculatedFare}&cu=INR`}
                  size={180}
                  viewBox={`0 0 180 180`}
                />
              </div>
              ...
              <button
                ...
                className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition uppercase tracking-widest text-[10px]"
              >
                Confirm Payment
              </button>
            </div>
