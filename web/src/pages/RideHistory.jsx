        ...
        <div className="flex p-1 bg-white rounded-xl shadow-sm border border-slate-100 mb-8 w-fit">
          <button
            ...
            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${
              activeTab === "taker" ? "bg-slate-900 text-white shadow-md" : "text-slate-500 hover:bg-slate-50"
            }`}
          >
            Passenger
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${
              activeTab === "giver" ? "bg-slate-900 text-white shadow-md" : "text-slate-500 hover:bg-slate-50"
            }`}
          >
            Driver
          </button>
        ...
        <div className="space-y-4">
              ...
              requests.map((req) => {
                ...
                return (
                  <div key={req.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    {req.status === "ACCEPTED" && (
                      <button
                        onClick={() => setPayModalData({ ...req, fare })}
          ...
          {activeTab === "giver" && (
            postedRides.length === 0 ? (
              <p className="text-slate-400 italic">You haven't posted any rides.</p>
            ) : (
              postedRides.map((ride) => {
                const seatPrice = ride.pricePerSeat || getDynamicCost(ride.distance);
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-500 text-sm mb-6">Scan with any UPI App</p>
              <div className="bg-white p-4 rounded-2xl border-2 border-slate-900 inline-block shadow-inner mb-6">
                <QRCode value={`upi://pay?pa=driver@upi&pn=RouteOptDriver&am=${payModalData.fare || 0}&cu=INR`} size={180} viewBox={`0 0 180 180`} />
              </div>
              <div className="bg-slate-50 p-4 rounded-xl mb-4 border border-slate-100">
                <p className="text-xs text-slate-400 uppercase font-bold tracking-wider">Total Amount</p>
                <p className="text-4xl font-black text-slate-900">₹{payModalData.fare || 0}</p>
              </div>
              <button onClick={() => { setPayModalData(null); alert("Payment done!"); }} className="w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition">I have Paid</button>
            </div>
          </div>
        </div>
      )}
