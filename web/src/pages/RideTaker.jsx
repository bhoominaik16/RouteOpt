...
const RideTaker = () => {
  ...
  // To book a ride, you MUST verify your identity (Aadhar)
  // You do NOT need License/RC/Medical for this role.
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full text-center border border-orange-100">
          ...
          <p className="text-slate-500 mt-2 mb-6">
            To ensure the safety of our community, you must verify your{" "}
            <strong>Aadhar Card</strong> before booking a ride.
          </p>
          <button
  ...
  const getCoordinates = async (address) => {
    try {
      ...
      const res = await axios.get(
        `https://photon.komoot.io/api/?q=${encodeURIComponent(address)}&limit=1&lat=19.07&lon=72.87`,
      );
      ...
      if (res.data.features.length > 0) {
        ...
        const city =
          feature.properties.city || feature.properties.state || "Unknown Area";
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!sourceData || !destData) {
        toast.error(
          "Could not locate places. Try adding 'Mumbai' to the name.",
          { id: toastId },
        );
        setLoading(false);
        return;
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (filters.genderPreference && user.gender && ride.driverGender) {
          if (ride.driverGender.toLowerCase() !== user.gender.toLowerCase())
            return;
        }

        // --- 🧠 ROUTE INTELLIGENCE ---
        const pickupMatch = getClosestPointOnRoute(
          sourceData,
          ride.routeGeometry,
        );
        const dropoffMatch = getClosestPointOnRoute(
          destData,
          ride.routeGeometry,
        );
      ...
      fetchedRides.sort((a, b) =>
        a.isSameOrg === b.isSameOrg ? 0 : a.isSameOrg ? -1 : 1,
      );
      setRides(fetchedRides);

      if (fetchedRides.length === 0) {
        toast.error("No matches found. Try specific landmarks.", {
          id: toastId,
        });
      } else {
        ...
        <aside className="w-full md:w-1/4 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 sticky top-24">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-md font-bold text-slate-700 mb-2 ml-1">
                  Pickup Location
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                  <span className="font-bold text-emerald-900 text-md">
                    Same-Gender Only
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform ${filters.genderPreference ? "left-5" : "left-1"}`}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <div>
                    <span className="font-bold text-blue-900 text-md block">
                      Same Institution
                    </span>
                    <p className="text-sm text-blue-700">
                      Verify via campus email
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() =>
                      setFilters({
                        ...filters,
                        sameInstitution: !filters.sameInstitution,
                      })
                    }
                    className={`w-10 h-5 rounded-full relative transition-colors ${filters.sameInstitution ? "bg-blue-600" : "bg-slate-300"}`}
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${filters.sameInstitution ? "left-5" : "left-1"}`}
                    />
                  </button>
                </div>
              </div>
          ...
          {!searching ? (
            <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-400 font-bold text-lg tracking-tight">
                Enter your route. We verify direction and proximity.
              </p>
            </div>
          ) : (
              ...
              {rides.length === 0 && !loading && (
                <div className="text-center p-10 text-slate-500">
                  ...
                  <span className="text-sm text-slate-400">
                    Try checking the spelling or adding "Mumbai" to the
                    location.
                  </span>
                </div>
              )}
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-6 pt-2">
                    ...
                    <div className="text-right flex flex-col justify-between items-end min-w-[120px]">
                      ...
                      <button
                        ...
                        className="text-emerald-600 font-bold text-sm hover:text-emerald-700 transition-colors flex items-center gap-1 mt-4 md:mt-0"
                      >
