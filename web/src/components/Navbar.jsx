...
const Navbar = () => {
  ...
  const [user, setUser] = useState(null); // We need a local state for the image/name because Profile saves to localStorage
  const [localData, setLocalData] = useState(
    JSON.parse(localStorage.getItem("user"))
  );
  const dropdownRef = useRef(null); // 1. Listen for Firebase Auth (Login Status)
  ...
  useEffect(() => {
    ...
    window.addEventListener("userUpdated", syncLocalData); // Also listen for storage events (multi-tab support)
  ...
  return (
    /* 🔥 UPDATED: Added shadow-lg, shadow-slate-200/50, and a visible border-b-2 border-emerald-500/10 */
    <nav className="sticky top-0 z-[1000] bg-white shadow-lg shadow-slate-200/50 border-b-2 border-emerald-500/10 px-8 py-4">
           {" "}
        ...
        <div className="flex items-center gap-6">
          ...
          {user ? (
            <>
              ...
              <Link
                to="/ride-selection"
                className="bg-slate-900 text-white px-5 py-2 rounded-full font-bold hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300"
              >
                                Start a Ride              {" "}
              </Link>
                           {" "}
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                    ...
                    <ul>
                      ...
                      <li>
                        ...
                        <Link
                          to="/profile"
                          className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition"
                        >
                                                   {" "}
                          <svg
                            className="w-4 h-4"
            ...
            <Link
              to="/auth"
              className="bg-slate-900 text-white px-6 py-2.5 rounded-full font-semibold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300"
            >
                            Get Started            {" "}
            </Link>
          )}
